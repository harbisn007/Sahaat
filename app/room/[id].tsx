...
export default function RoomScreen() {
  ...
  useEffect(() => {
    if (roomData && username) {
      const participant =
        roomData.participants.find((p) => p.userId === userId) ||
        roomData.participants.find((p) => p.username === username);
      if (participant) {
  ...
  }, [roomData, username, userId, userRole, isApproved]);
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          ...
          {(role === 'admin' || role === 'moderator') && (
            <TouchableOpacity
              ...
              onPress={async () => {
                try {
                  const response = await fetch('https://sahaat-production.up.railway.app/mod/pin-room', {
                    method: 'POST',
        ...
        >
          <View style={{
            ...
            <Text style={{
              ...
              • يمكنك ان تلعب كشاعر او كمستضيف ومدير للملعبة ،يمكن استضافة شاعرين.{"\n"}
              ...
              • استخدم زر شيلوها لتجعل الصفوف تردد البيت حتى تجهز الرد او البيت التالي ( الصفوف(شيلوها) ستردد اخر بيت مرسل من زر طاروق من اي من الشاعرين){"\n"}
              ...
              • ارسل دعوة عامة تظهر في صفحة ساحات الطواريق او شارك رابط ساحتك مع الاصدقاء.{"\n"}
              • استخدم المثبتة لتثبيت بيت مجاراة او لغز او ماتشاء تثبيته.{"\n"}
              • الابلاغ عن الرسائل المسيئة بالضغط المستمر على اسم المرسل للرسالة نفسها.
            </Text>
            <Text style={{
              color: '#9CA3AF',
              fontSize: 14,
